...
function ProductList({ onHomeClick }) {
  ...
  const handleAddtoCart = (plant) => {
    dispatch(addItem({ ...plant, id: plant.name }));
        ...
        <div className="product-grid">
          {plantsArray.map((categoryObj) => (
            <div key={categoryObj.category}>
              <h2 style={{ margin: "15px" }}>{categoryObj.category}</h2>
              <div className="plant-catagory">
                {categoryObj.plants.map((plant) => (
                  ...
                  >
                    ...
                    ></img>
                    <p style={{ margin: "10px" }}>{plant.cost}</p>
                    <p style={{ margin: "10px" }}>{plant.description}</p>
                    <button
                      className="product-button"
                      onClick={() => handleAddtoCart(plant)}
                      style={{
                        background: addedToCart[plant.name]
                          ? "#888"
                          : "#4CAF50",
                        color: "white",
                        border: "none",
